...
class Data_Plotter:
    ...
    def plot_results(self):
        ...
        for a in self.a_values:
            ...
            k_entropy_results = []
            for k in self.k_values:
                ax2.annotate(f"{self.results[k][a]:0.3f}", xy=(k, self.results[k][a]+self.results[k][a]*0.01), fontsize=6)
        ...
        for k in self.k_values:
            ...
            a_entropy_results = []
            for a in self.a_values:
                ax2.annotate(f"{self.results[k][a]:0.3f}", xy=(a, self.results[k][a]+self.results[k][a]*0.01), fontsize=6)
